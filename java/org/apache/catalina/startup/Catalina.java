...
public class Catalina {
     ...
     * 读取config/server.xml或server-embed.xml文件，实例化Server/Service/Connector/Engine/Host等组件
     * 并且调用Lifecycle#init()完成初始化动作，以及发出INITIALIZING、INITIALIZED事件
     */
    public void load() {
        ...
        // Create and execute our Digester
        // 创建Digester实例解析conf/server.xml文件，定义每个xml标签该解析成什么类
        ...
        try {
            ...
            if (inputStream == null) {
                try {
                    // 优先加载conf/server.xml，找不到再去加载server-embed.xml
                    inputStream = getClass().getClassLoader()
            ...
            // 如果server.xml和server-embed.xml均不存在，则直接返回
            ...
            try {
                ...
                // 把Catalina作为一个顶级实例
                digester.push(this);
                // 解析过程会实例化各个组件，如Server、Container、Connector等
